...
public class UploadControllerTest {
    ...
    public void fileTooLargeReturnsBadRequest() throws Exception {
        ...
        mvc.perform(multipart("/api/files/upload").file(file).with(csrf())).andExpect(status().isBadRequest());
    }
    ...
    public void passwordTooShortReturnsBadRequest() throws Exception {
        ...
        mvc.perform(multipart("/api/files/upload").file(file).param("password","123").with(csrf())).andExpect(status().isBadRequest());
    }
    ...
    public void expirationTooLongReturnsBadRequest() throws Exception {
        MockMultipartFile file = new MockMultipartFile("file","a.txt", MediaType.TEXT_PLAIN_VALUE, "ok".getBytes());
        mvc.perform(multipart("/api/files/upload").file(file).param("expirationDays","10").with(csrf())).andExpect(status().isBadRequest());
    }
    ...
    public void validUploadReturnsCreatedAndToken() throws Exception {
        MockMultipartFile file = new MockMultipartFile("file","a.txt", MediaType.TEXT_PLAIN_VALUE, "ok".getBytes());
        FileMetadata meta = new FileMetadata("tok123","a.txt", 2, LocalDateTime.now().plusDays(7), List.of("tag1"));
        when(uploadService.store(any(), anyLong(), any())).thenReturn(meta);

        mvc.perform(multipart("/api/files/upload").file(file).with(csrf())).andExpect(status().isCreated()).andExpect(jsonPath("$.token").value("tok123"));
    }
}
